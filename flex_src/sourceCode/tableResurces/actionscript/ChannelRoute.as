...
{
	registerClassAlias("channelroute.model.Circuit",Circuit);
...
{
	var rmObj:RemoteObject = new RemoteObject("channelRouteForm");
...
protected function controlBar_controlEditHandler(event:Event):void
{
	if(dataGrid.selectedItems.length>0){
		...
		cit.portserialno1.text = dataGrid.selectedItem.portserialno1;
		cit.portserialno2.text = dataGrid.selectedItem.portserialno2;
...
private function closeHandler(e:CloseEvent):void{
	...
	if(e.detail == Alert.YES){
		var obj1:RemoteObject = new RemoteObject("channelRouteForm");
		obj1.endpoint=ModelLocator.END_POINT;
		obj1.deleteCircuit(dataGrid.selectedItem);
		obj1.addEventListener(ResultEvent.RESULT,refreshHandle);
...
//	}
	//弹出查询框和赋值
//	MyPopupManager.addPopUp(circuitTitle,false,null);
//	circuitTitle.circuit=circuit;
}
...
private function toolbar_ExportExcelHandler(event:Event):void{
	...
	fefs.titles = new Array("序号","电路编号","电路名称","A端局站","Z端局站","业务类型","网管A端端口",
		"网管Z端端口","资源A端端口","资源Z端端口","A端端口时隙","Z端端口时隙","速率","开通时间");				
	fefs.exportTypes = "电路";
