...
import sourceCode.sysGraph.views.DetailPressure;
...
private var strategyArr:ArrayCollection = new ArrayCollection([
	"--策略选择--","时延策略","关键业务策略"]);      //最短路径策略  没有"压力均衡策略",
[Bindable]
...
private var pressure:DetailPressure;
...
{      
	...
	var s:Boolean=false;
	var roll:RemoteObject = new RemoteObject("fiberWire");
	roll.endpoint = ModelLocator.END_POINT;
	roll.showBusyCursor = true;
	roll.addEventListener(ResultEvent.RESULT,getConditonqs);	
	roll.hsdata(s,SysOrgMapL.AA);
...
//						}	
					...
					{	
						if(element is Link)
						{
							...
							if(link.toNode.id.toString()==arr[i+1]&&link.fromNode.id.toString()==arr[i]){
								...
								link.setStyle(Styles.CONTENT_TYPE, Consts.CONTENT_TYPE_DEFAULT_VECTOR);
								link.setStyle(Styles.VECTOR_FILL_ALPHA, 0.6);
								link.setStyle(Styles.VECTOR_FILL_COLOR, 0x008000);
								systemorgmap1.elementBox.selectionModel.appendSelection(link);//显示不完全 有问题 mawj
							}
							if(link.toNode.id.toString()==arr[i]&&link.fromNode.id.toString()==arr[i+1]){
			...
			{  systemorgmap1.elementBox.selectionModel.clearSelection();
				...
				if(fla==true)
				{    //abc_route.removeAll();
					Alert.show("birthroutes")
					var ro:RemoteObject = new RemoteObject("topolink");
					ro.endpoint = ModelLocator.END_POINT;
					ro.showBusyCursor = true;
					ro.addEventListener(ResultEvent.RESULT,getConditon);
					ro.birthroutes1(createbusiness.group.getItemAt(numid).name.toString(),createbusiness.group.getItemAt(numid).start.toString(),createbusiness.group.getItemAt(numid).end.toString(),ServDownList.selectedIndex);  		
				}
				if(fla==false)
				{  
					//abc_route.removeAll();
					...
					ro.birthroutess1(treename,myData.getItemAt(0).PA.toString(),myData.getItemAt(0).PB.toString(),ServDownList.selectedIndex);  	
		...
		private function getConditon1(event:ResultEvent):void
			...
			{
				//Shareds.ses=busGrid.selectedItem.name.toString();	
				    Shareds.alla=true;
			...
			{
				DetailPressure.equipInfojs=event.result as ArrayCollection;
