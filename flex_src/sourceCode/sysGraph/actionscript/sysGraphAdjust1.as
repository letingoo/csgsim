...
{    
	...
	ro.hsdata(s,SysOrgMapL1.AA.toString());//往后台传输复用段编号，s为true表示展开页面不显示全网数据而展示该复用段数据
...
//						}	
			...
			{  	
				...
				for(var i:int=0;i<createbusiness.group.length;i++)
				{
					if(tree.selectedItem.@name==createbusiness.group.getItemAt(i).name.toString())//判断选择的业务是否为新添加的业务
					{    fla=true;
						numid=i;
						break;
					}
