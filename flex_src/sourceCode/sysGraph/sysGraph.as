			...
			//			cob_add.visible=true;cob_add.width=50;
				...
				private function cob_todoAnaly():void{
					...
					if(str == "成环率分析"){
						...
						lab.visible=true;
						selectflag = 3;
						var rate:Number = Circlerate.main(info_array1,nodename_array);
						...
						if(flag=="1"){
							...
							lab.text = currSystemName+" 成环率分析"+" 成环率"+((info_array1.length-C_Analysis.length)/info_array1.length*100).toFixed(2)+ "%"; 
						...
						*/
						...
						var test2:Array=new Array();
						var test3:Array=new Array();
						
						test1[0]=new Array(0,1,1,0);
						test1[1]=new Array(1,0,0,1);
						test1[2]=new Array(1,0,0,1);
						test1[3]=new Array(0,1,1,0);
						...
						*/
						...
						/*
						for(var k:int=0;k<info_array.length;k++){
						if(nodename_array[k]=="1006-清苑"){
						str=nodename_array[k]+",,,";
						for(var i:int=0;i<info_array.length;i++){
						if(info_array[k][i]>0){
						str=str+nodename_array[i]+",,,";
						}
						}
						Alert.show(str);
						}
						if(nodename_array[k]=="2014-邯供"){
						str=nodename_array[k]+",,,";
						for(var i:int=0;i<info_array.length;i++){
						if(info_array[k][i]>0){
						str=str+nodename_array[i]+",,,";
						...
						*/
						...
						{
						var node:Node=element as Node;
						for(var critnum:int=0;critnum<K_Analysis.length;critnum++){
						if(node.id==K_Analysis[critnum].ID){
