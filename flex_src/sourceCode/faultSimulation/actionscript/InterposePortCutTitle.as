...
protected function btn_clickHandler(event:MouseEvent):void
{
	...
	if(this.title == "添加"){
		...
		interposeModel.faulttype = "AT10000001";
		interposeModel.faulttypeid = "AT10000001";
		...
		//割接端口
		interposeModel.cutportcode=cutportcode;//割接用端口
		interposeModel.cutport = oldport;//当前选中端口
		remoteObject.addEventListener(ResultEvent.RESULT,addInterposeResult);
		Application.application.faultEventHandler(remoteObject);
		remoteObject.addInterposePortCut(interposeModel);
	}
}
/**
 *添加经数据交互后的的界面提示处理
