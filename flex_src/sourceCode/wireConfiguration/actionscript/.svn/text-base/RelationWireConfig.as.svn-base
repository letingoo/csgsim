...
import sourceCode.roomGraph.model.OdfOdm;
import sourceCode.wireConfiguration.model.DMModel;
import sourceCode.wireConfiguration.model.DdfDdmModule;
import sourceCode.wireConfiguration.model.EquipmentModule;
import sourceCode.wireConfiguration.model.FiberLinesModel;
import sourceCode.wireConfiguration.model.FiberModule;
import sourceCode.wireConfiguration.model.MatrixModel;
import sourceCode.wireConfiguration.model.ODFSUBMODULE;
import sourceCode.wireConfiguration.model.ODMMODULE;
import sourceCode.wireConfiguration.model.OcableGraphicModule;
import sourceCode.wireConfiguration.model.VolumeModel;
import sourceCode.wireConfiguration.model.wireConfigurationGraphicsDataModule;
import sourceCode.wireConfiguration.views.DDFPortRelation;
import sourceCode.wireConfiguration.views.viewDdfPortDetails;
import sourceCode.wireConfiguration.views.viewLogicportDetails;
import sourceCode.wireConfiguration.views.viewOdfPortDetails;
import sourceCode.wireStats.model.DDFModel;
...
[bindable]
private var dgPortData:String;
[bindable]
private var dgOdfPortData:String;
[bindable]
private var dgDdfPortData:String;
...
public function initApp():void  { 
	...
	DemoUtils.initNetworkToolbarForSysGraph(toolbar, network,"默认模式");
	...
	//导出连接关系
		private function exportExcel():void{
			...
			for(var i:int = 0; i < this.ODFLinesData.length; i++){
				...
				m.PORTSERIALNO1 = this.ODFLinesData[i].PORTSERIALNO1;
				m.PORTSERIALNO2 = this.ODFLinesData[i].PORTSERIALNO2;
				m.PORTTYPE1 = this.ODFLinesData[i].PORTTYPE1;
				m.PORTTYPE2 = this.ODFLinesData[i].PORTTYPE2
			...
			for(var j:int = 0; j < this.DDFLinesData.length; j++){
				...
				m1.PORTSERIALNO1 = this.DDFLinesData[j].PORTSERIALNO1;
				m1.PORTSERIALNO2 = this.DDFLinesData[j].PORTSERIALNO2;
				m1.PORTTYPE1 = this.DDFLinesData[j].PORTTYPE1;
				m1.PORTTYPE2 = this.DDFLinesData[j].PORTTYPE2;
			...
			if(ac.length > 0){
				...
				var titles:Array = new Array("A端端口", "Z端端口", "Z’DDF端口","A端端口类型","Z端端口类型","Z’DDF端口类型","业务名称","开通时间");
		...
		private function createLinkHandler(link:Link):void{
			...
			if(fromType != "" && toType != ""){ 
				...
				if(link.fromNode.getClient("status") == "ZY13100203" || link.toNode.getClient("status") == "ZY13100203" ){
					Alert.show("损坏的端口不能再做配线关系！","提示");
					elementBox.removeByID(_tempLinkID);
					return;
				}
				
				if(link.fromNode.getClient("linecount") >=2 || link.toNode	.getClient("linecount") >= 2){
					Alert.show("端口配线关系不能多于两条！","提示");
					elementBox.removeByID(_tempLinkID);
					return;
				}
				
				if(fromType != "fiber" && toType != "fiber"){
					if(Follower(link.fromNode).getClient("stationcode") !=  Follower(link.toNode).getClient("stationcode") ){
				...
				if(fromType == "ZY03070401" || fromType == "ZY03070402"){ //线路端口、光支路端口 
					if(toType == "fiber" || toType == "ZY23010499"){      //判断对端是光纤还是ODF
						if(toType == "fiber"){  //如果对端是光纤，则要判断要存入到哪一端
							...
							link.setClient("code",toNode + ":" + fromNode);
							if(!isVolume){
							...
							openVoleume("设备端口(A)","光纤端口(Z)",link.fromNode.name,link.toNode.name,acPortsA,acPortsZ,field+"-z");
						}else{		//如果对端是ODF模块
							this.markType = "matrix";
							this.markValue = "ODF";
							link.setClient("linktype","matrix");
							link.setClient("code",fromNode + ":" + toNode);
							if(!isVolume){
							...
							packserial = link.fromNode.parent.parent.getClient("packcode").split("-")[2]
							acPortsA = searchPorts(parentNameA,frameserial,slotserial,packserial);
							acPortsZ = searchPorts(parentNameZ,"","","","odf");
							openVoleume("设备端口(A)","ODF端口(Z)",link.fromNode.name,link.toNode.getClient("name"),acPortsA,acPortsZ,"",false,"equipment-"+parentNameA+"-ODF-"+parentNameZ,null,getLocation(parentNameZ));
						}
						bool = false;
					}
					...
					if(toType == "ZY13010499"){        //如果对端是DDF模块
						...
						acPortsA = searchPorts(parentNameA,frameserial,slotserial,packserial);
						acPortsZ = searchPorts(parentNameZ);
						openVoleume("设备端口(A)","DDF端口(Z)",link.fromNode.name,link.toNode.name,acPortsA,acPortsZ,"",false,"equipment-"+parentNameA+"-DDF-"+parentNameZ,null,getLocation(parentNameZ));
					}
					...
					if(toType == "ZY03070401" || toType == "ZY03070402" || toType == "fiber" || toType == "ZY23010499"){
						if(toType == "fiber"){
							...
							parentNameZ = link.toNode.parent.parent.getClient("code");
							openVoleume("ODF端口(A)","光纤端口(Z)",link.fromNode.getClient("name"),link.toNode.name,searchPorts(parentNameA,"","","","odf"),searchPorts(parentNameZ),field+"-z",false,"ODF-"+parentNameA+"-ocable-"+parentNameZ,getLocation(parentNameA));
						}
						if(toType == "ZY03070401" || toType == "ZY03070402"){
							...
							acPortsA = searchPorts(parentNameA,"","","","odf");
							acPortsZ = searchPorts(parentNameZ,frameserial,slotserial,packserial);
							openVoleume("ODF端口(A)","设备端口(Z)",link.fromNode.getClient("name"),link.toNode.name,acPortsA,acPortsZ,"",false,"ODF-"+parentNameA+"-equipment-"+parentNameZ,getLocation(parentNameA));
						}
						if(toType == "ZY23010499"){
							this.markType = "matrix";
							this.markValue = "ODF";
							...
							if(parentNameA == parentNameZ){
								openVoleume("ODF端口(A)","ODF端口(Z)",link.fromNode.getClient("name"),link.toNode.getClient("name"),searchPorts(parentNameA,"","","","odf"),searchPorts(parentNameZ,"","","","odf"),"",false,"ODF-"+parentNameZ,getLocation(parentNameZ));
							}else{
								tempPoint1 = getLocation(parentNameA);
								tempPoint2 = getLocation(parentNameZ);
								openVoleume("ODF端口(A)","ODF端口(Z)",link.fromNode.getClient("name"),link.toNode.getClient("name"),searchPorts(parentNameA,"","","","odf"),searchPorts(parentNameZ,"","","","odf"),"",false,"ODF-"+parentNameA+"-ODF-"+parentNameZ,tempPoint1,tempPoint2);
							}
						}
						bool = false;
					}
					...
					if(toType == "ZY03070403" || toType == "ZY13010499"){
						...
						if(toType == "ZY13010499"){
							...
							parentNameZ = Follower(link.toNode).parent.getClient("code");
							if(parentNameA == parentNameZ){
								openVoleume("DDF端口(A)","DDF端口(Z)",link.fromNode.name,link.toNode.name,searchPorts(parentNameA),searchPorts(parentNameZ),"",false,"DDF-"+parentNameA,getLocation(parentNameA));
							}else{
								tempPoint1 = getLocation(parentNameA);
								tempPoint2 = getLocation(parentNameZ);
								openVoleume("DDF端口(A)","DDF端口(Z)",link.fromNode.name,link.toNode.name,searchPorts(parentNameA),searchPorts(parentNameZ),"",false,"DDF-"+parentNameA+"-DDF-"+parentNameZ,tempPoint1,tempPoint2);
							}
						}else{
							parentNameA = Follower(link.fromNode).parent.getClient("code");
							parentNameZ = link.toNode.parent.parent.parent.getClient("code");
							...
							acPortsZ = searchPorts(parentNameZ,frameserial, slotserial,packserial);
							openVoleume("DDF端口(A)","设备端口(Z)",link.fromNode.name,link.toNode.name,acPortsA,acPortsZ,"",false,"DDF-"+parentNameA+"-equipment-"+parentNameZ,getLocation(parentNameA));
						}
					}
					...
					if(toType == "ZY03070401" || toType == "ZY03070402" || toType == "ZY23010499"){
						...
						if(toType == "ZY03070401" || toType == "ZY03070402"){
							...
							acPortsA = searchPorts(parentNameA);
							acPortsZ = searchPorts(parentNameZ,frameserial, slotserial,packserial);
							openVoleume("光纤端口(A)","设备端口(Z)",link.fromNode.name,link.toNode.name,acPortsA,acPortsZ,field+"-a");
						}
						if(toType == "ZY23010499"){
							if(!isVolume){
							}
							...
							parentNameZ = Follower(link.toNode).parent.getClient("code");
							openVoleume("光纤端口(A)","ODF端口(Z)",link.fromNode.name,link.toNode.getClient("name"),searchPorts(parentNameA),searchPorts(parentNameZ,"","","","odf"),field+"-a",false,"ocable-"+parentNameA+"-ODF-"+parentNameZ,null,getLocation(parentNameZ));
						}
					}
				}
					...
					if(!isVolume){
						...
						if(fromType == "ZY03070401" || fromType == "ZY03070402" || fromType == "ZY03070403"){
							...
							if(toType == "fiber"){				//光纤与ODF连接关系时做特殊处理
								...
								for(var i:int=0; i < acAllPorts.length; i++){ //更新集合里ODF端口连接数量
									if(acAllPorts[i].id == link.fromNode.parent.getClient("code")){
										var tempAC:ArrayCollection = acAllPorts[i].data as ArrayCollection;
										for each(var obj:Object in tempAC){
											if(obj.portcode == link.fromNode.getClient("code")){
												obj.linecount = link.fromNode.getClient("linecount") + 1;
												break;
											}
										}
										break;
							...
							if(toType == "ZY23010499"){   //光纤与ODF连接关系时做特殊处理
								...
								for(var i:int=0; i < acAllPorts.length; i++){ //更新集合里ODF端口连接数量
									if(acAllPorts[i].id == link.toNode.parent.getClient("code")){
										var tempAC:ArrayCollection = acAllPorts[i].data as ArrayCollection;
										for each(var obj:Object in tempAC){
											if(obj.portcode == link.toNode.getClient("code")){
												obj.linecount = link.toNode.getClient("linecount") + 1;
												break;
											}
										}
										break;
		...
		private function setNodeColor(node:Node, type:String,isAdd:Boolean):void{
			...
			if(type == "fiber"){
				...
				if (count == 0){
					if(type=="ODF"){
						if(node.getClient("castdirection") =="1"){
							node.image="ODFVPORT"
						}else{
							node.image="odf_port"
						}
		...
		private function openVoleume(lblNameA:String,lblNameZ:String,startA:String,startZ:String,portsA:ArrayCollection,portsZ:ArrayCollection,field:String="",del:Boolean=false,moduleName:String="",point:Point=null,point1:Point = null):void{
			...
			if(network.selectionModel.count > 0)
			{
				...
				if(element is Follower && element.layerID == layer1.id ){
					...
					if(element.getClient("type") == "ocable"){
						var itemUp:ContextMenuItem = new ContextMenuItem("旋转向上",true);
						itemUp.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT,revolveOcableHandler);
						var itemDown:ContextMenuItem = new ContextMenuItem("旋转向下");
						itemDown.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT,revolveOcableHandler);
						var itemLeft:ContextMenuItem = new ContextMenuItem("旋转向左");
						itemLeft.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT,revolveOcableHandler);
						var itemRight:ContextMenuItem = new ContextMenuItem("旋转向右");
						itemRight.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT,revolveOcableHandler);
						network.contextMenu.customItems = [itemUp,itemDown,itemLeft,itemRight];
						var fiberdetail:ContextMenuItem = new ContextMenuItem("查看光纤信息");
						fiberdetail.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT,showPortHandler);
						network.contextMenu.customItems = [fiberdetail,itemrevoke];
					}else if(element.getClient("type")=="odf"){
						var itemodfdetail:ContextMenuItem = new ContextMenuItem("查看ODF端口信息");
						itemodfdetail.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT,showOdfPortHandler); 
						network.contextMenu.customItems = [itemodfdetail,itemrevoke];
					}else if(element.getClient("type")=="ddf"){
						var itemddfdetail:ContextMenuItem = new ContextMenuItem("查看DDF端口信息");
						itemddfdetail.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT,showDdfPortHandler);
						network.contextMenu.customItems = [itemddfdetail,itemrevoke];
					}else if(element.getClient("type") == "equipment"){
						var itemequipdetail:ContextMenuItem = new ContextMenuItem("查看设备端口信息");
						itemequipdetail.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT,showequipHandler);
						network.contextMenu.customItems = [itemequipdetail,itemrevoke];
					}else{
				...
				}else if(element is Follower && element.layerID == layer3.id && element.getClient("porttype") != null){
					...
					if(element.getClient("porttype") == "fiber" ){
						...
						network.contextMenu.customItems = [itemShowRes,itemFiberRoute,itemOcableRoute];
					}else if( Follower(element).getClient("porttype") == "ZY13010499" ){
						var itemddfcircuit:ContextMenuItem = new ContextMenuItem("查看端到端连接关系",true);
						itemddfcircuit.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT,showDDFCircuitHandler);
						network.contextMenu.customItems = [itemShowRes,itemddfcircuit];
						...
						network.contextMenu.customItems = [itemShowRes,itemocableroute];
					}else{
...
//					network.setDefaultInteractionHandlers();
		...
		private function showOcableRouteHandler(even:ContextMenuEvent):void{
			...
			ro.endpoint= ModelLocator.END_POINT;
			ro.showBusyCursor = true;
			ro.getOcableRouteByOdfPort(port.getClient("code"));
		...
		private function showFiberOcableRouteHandler(event:ContextMenuEvent):void{
			...
			ro.endpoint= ModelLocator.END_POINT;
			ro.showBusyCursor = true;
			ro.getOcableRouteByFiber(port.getClient("code"));
			ro.addEventListener(ResultEvent.RESULT,joinResultHandler);
		}
		...
		private function showOdfPortHandler(event:ContextMenuEvent):void{
			if(network.selectionModel.count > 0){
				var follower:Follower = network.selectionModel.selection.getItemAt(0);
				var odfodmcode:String = follower.getClient("code");
				var vodf:viewOdfPortDetails = new viewOdfPortDetails();
				vodf.odfodmcode =odfodmcode;
				vodf.title = follower.name+"—端口信息";
				MyPopupManager.addPopUp(vodf);
			}
		}
		
		private function showDdfPortHandler(event:ContextMenuEvent):void{
			if(network.selectionModel.count > 0){
				var follower:Follower = network.selectionModel.selection.getItemAt(0);
				var ddfddmcode:String = follower.getClient("code");
				var vddf:viewDdfPortDetails = new viewDdfPortDetails();
				vddf.ddfddmcode =ddfddmcode;
				vddf.title = follower.name+"—端口信息";
				MyPopupManager.addPopUp(vddf);
			}
		}
		
		private function showequipHandler(event:ContextMenuEvent):void{
			if(network.selectionModel.count > 0){
				var follower:Follower = network.selectionModel.selection.getItemAt(0);
				var equipcode:String = follower.getClient("code");
				var vld:viewLogicportDetails = new viewLogicportDetails();
				vld.equipcode = equipcode;
				vld.title = follower.name+"—端口信息";
				MyPopupManager.addPopUp(vld);
			}
		}
		
		private function revolveOcableHandler(event:ContextMenuEvent):void{
			var element:Element = network.selectionModel.selection.getItemAt(0);
			
			var fibermodule:FiberModule = new FiberModule();
			fibermodule.ocableCode = element.getClient("code");
			fibermodule.name_std = element.name;
			fibermodule.a_point = element.getClient("a_point");
			fibermodule.z_point = element.getClient("z_point");
			fibermodule.a_pointtype = element.getClient("a_pointtype");
			fibermodule.z_pointtype = element.getClient("z_pointtype");
			state = element.getClient("state");
			var fiberdata:ArrayCollection = searchPorts(fibermodule.ocableCode);
			
			var ogm:OcableGraphicModule = new OcableGraphicModule();
			ogm.FIBER_MODULE_DATA = fibermodule;
			ogm.FIBER_DATA=fiberdata;
			this.removeModule(fibermodule.ocableCode,"code","ocable",twaver.Consts.PROPERTY_TYPE_CLIENT);
			
			if(event.target.caption == "旋转向上"){
				this.CreateFiberModule(200,400,276,ogm,"Up","Single",true);
			}else if(event.target.caption == "旋转向下"){
				this.CreateFiberModule(200,30,276,ogm,"Down","Single",true);
			}else if(event.target.caption == "旋转向左"){
				this.CreateFiberModule(100,300,276,ogm,"Left","Single",true);
			}else if(event.target.caption == "旋转向右"){
				this.CreateFiberModule(500,300,276,ogm,"Right","Single",true);
			}
		...
		private function showDDFCircuitHandler(event:ContextMenuEvent):void{
			...
			ro.endpoint= ModelLocator.END_POINT;
			ro.showBusyCursor = true;
			ro.getDdfPortRelation(port.parent.getClient("dmcode"),port.getClient("code"));
		...
		//查看光纤详细信息。
		private function showPortHandler(event:ContextMenuEvent):void{
			if(network.selectionModel.count > 0){
				var follower:Follower = network.selectionModel.selection.getItemAt(0);	

				if(follower.getClient("type")=="ocable")
				{
					var ocableCode:String = follower.getClient("code");
					var vfd:viewFiberDetails = new viewFiberDetails();
					vfd.sectioncode = ocableCode;
					vfd.title = follower.name+"—纤芯信息";
		...
		public function delMatrixsLink(code:String,type:String):void{
			var portA:String = code.split(":")[0];
			var portZ:String = code.split(":")[1];
			if(type == "DDF"){
				for(var i:int = 0; i < DDFLinesData.length; i++){
					if((portA == DDFLinesData[i].PORTSERIALNO1 && portZ == DDFLinesData[i].PORTSERIALNO2)||
						(portA == DDFLinesData[i].PORTSERIALNO2 && portZ == DDFLinesData[i].PORTSERIALNO1))
						DDFLinesData.removeItemAt(i);
				}
			}else{
				for(var j:int = 0; j < ODFLinesData.length; j++){
					if((portA == ODFLinesData[j].PORTSERIALNO1 && portZ == ODFLinesData[j].PORTSERIALNO2)||
						(portA == ODFLinesData[j].PORTSERIALNO2 && portZ == ODFLinesData[j].PORTSERIALNO1))
					{
			...
			if(network.selectionModel.count > 0){
				...
				if(link.getClient("linktype") == "matrix"){
					...
					if(fromType == "ZY13010499" && toType == "ZY13010499"){//两端是DDF
						...
						parentNameZ = Follower(link.toNode).parent.getClient("code");
						if(parentNameA == parentNameZ){
							openVoleume("DDF端口(A)","DDF端口(Z)",link.fromNode.name,link.toNode.name,searchPorts(parentNameA),searchPorts(parentNameZ),"",true,"DDF-"+parentNameZ,getLocation(parentNameZ));
						}else{
							tempPoint = getLocation(parentNameA);
							tempPoint1 = getLocation(parentNameZ);
							openVoleume("DDF端口(A)","DDF端口(Z)",link.fromNode.name,link.toNode.name,searchPorts(parentNameA),searchPorts(parentNameZ),"",true,"DDF-"+parentNameA+"-DDF-"+parentNameZ,tempPoint,tempPoint1);
						}
						...
						parentNameA = Follower(link.fromNode).parent.getClient("code");;
						parentNameZ = Follower(link.toNode).parent.getClient("code");
						if(parentNameA == parentNameZ){ //同一个模块内配线
							openVoleume("ODF端口(A)","ODF端口(Z)",link.fromNode.getClient("name"),link.toNode.getClient("name"),searchPorts(parentNameA,"","","","odf"),searchPorts(parentNameZ,"","","","odf"),"",true,"ODF-"+parentNameA,getLocation(parentNameA));
						}else{
							tempPoint = getLocation(parentNameA);
							tempPoint1 = getLocation(parentNameZ);
							openVoleume("ODF端口(A)","ODF端口(Z)",link.fromNode.getClient("name"),link.toNode.getClient("name"),searchPorts(parentNameA,"","","","odf"),searchPorts(parentNameZ,"","","","odf"),"",true,"ODF-"+parentNameA+"-ODF-"+parentNameZ,tempPoint,tempPoint1);
						}
						...
						acPortsA = searchPorts(parentNameA);
						acPortsZ = searchPorts(parentNameZ,frameserial,slotserial,packserial);
						openVoleume("DDF端口(A)","设备端口(Z)",link.fromNode.name,link.toNode.name,acPortsA,acPortsZ,"",true,"DDF-"+parentNameA +"-equipment-"+parentNameZ,getLocation(parentNameA));
						...
						acPortsA = searchPorts(parentNameA,frameserial,slotserial,packserial);
						acPortsZ = searchPorts(parentNameZ);
						openVoleume("设备端口(A)","DDF端口(Z)",link.fromNode.name,link.toNode.name,acPortsA,acPortsZ,"",true,"equipment-"+parentNameA + "-DDF-"+parentNameZ,null,getLocation(parentNameZ));
						...
						acPortsA = searchPorts(parentNameA,frameserial,slotserial,packserial);
						acPortsZ = searchPorts(parentNameZ,"","","","odf");
						openVoleume("设备端口(A)","ODF端口(Z)",link.fromNode.name,link.toNode.getClient("name"),acPortsA,acPortsZ,"",true,"equipment-"+parentNameA+"-ODF-"+parentNameZ,null,getLocation(parentNameZ));
						...
						acPortsA = searchPorts(parentNameA,"","","","odf");
						acPortsZ = searchPorts(parentNameZ,frameserial,slotserial,packserial);
						openVoleume("ODF端口(A)","设备端口(Z)",link.fromNode.getClient("name"),link.toNode.name,acPortsA,acPortsZ,"",true,"ODF-"+parentNameA+"-equipment-"+parentNameZ,getLocation(parentNameA));
				}
				...
				if(porttype1 == "fiber"){
					...
					odfportcode = link.toNode.id.toString().split("-")[1];
					...
					odfportcode = link.fromNode.id.toString().split("-")[1];
				}
				if(apointtype == "1"){
				...
				if(!isVolume){
					ro.delRelationFromFiber(fiberCode,field,odfportcode);
					delFibersLink(link.getClient("code"));
				}
				markType = "fiber";
				if(fromType == "ZY03070401" || fromType == "ZY03070402"){
					...
					if(toType == "fiber"){
						...
						acPortsA = searchPorts(parentNameA,frameserial,slotserial,packserial);
						acPortsZ = searchPorts(parentNameZ);
						openVoleume("设备端口(A)","光纤端口(Z)",link.fromNode.name,link.toNode.name,acPortsA,acPortsZ,field+"-z",true);
					}else if(toType == "ZY23010499"){
						markType == "matrix";
						parentNameA = link.fromNode.parent.parent.parent.getClient("code");
						...
						acPortsA = searchPorts(parentNameA,frameserial,slotserial,packserial);
						acPortsZ = searchPorts(parentNameZ,"","","","odf");
						openVoleume("设备端口(A)","ODF端口(Z)",link.fromNode.name,link.toNode.getClient("name"),acPortsA,acPortsZ,"",true,"ODF-"+parentNameZ,getLocation(parentNameZ));
							}
					}else if(fromType == "fiber" && toType == "ZY23010499"){
						parentNameA = link.fromNode.parent.parent.getClient("code");
						parentNameZ = Follower(link.toNode).parent.getClient("code");
						openVoleume("光纤端口(A)","ODF端口(Z)",link.fromNode.name,link.toNode.getClient("name"),searchPorts(parentNameA),searchPorts(parentNameZ,"","","","odf"),field+"-a",true,"ocable-"+ parentNameA+"-ODF-"+parentNameZ,null,getLocation(parentNameZ));
					}else if(fromType == "ZY23010499" && toType == "fiber"){
						parentNameA = Follower(link.fromNode).parent.getClient("code");
						parentNameZ = link.toNode.parent.parent.getClient("code");
						openVoleume("ODF端口(A)","光纤端口(Z)",link.fromNode.getClient("name"),link.toNode.name,searchPorts(parentNameA,"","","","odf"),searchPorts(parentNameZ),field+"-z",true,"ODF-"+parentNameA+"-ocable-"+parentNameZ,getLocation(parentNameA));
					}
				}
		...
		public function createMatrixLink(type:String):void{
			...
			if(type == "DDF"){
				for(var i:int =0;i<DDFLinesData.length;i++){
					elementa = findElementByID(DDFLinesData.getItemAt(i).PORTSERIALNO1);
					elementb = findElementByID(DDFLinesData.getItemAt(i).PORTSERIALNO2);
					if(elementa && elementb){
						...
						link.setClient("linktype","matrix");
						link.setClient("code",elementa.id.toString().split("-")[1]+":"+elementb.id.toString().split("-")[1]);
						link.layerID=layer5.id;
						link.setStyle(Styles.SELECT_STYLE,Consts.SELECT_STYLE_BORDER);
						link.setStyle(Styles.SELECT_COLOR,"0x00FF00");
						link.setStyle(Styles.SELECT_WIDTH,'5');
					}
				}
			}else{
				for(var j:int =0;j<ODFLinesData.length;j++){
					elementa = findElementByID(ODFLinesData.getItemAt(j).PORTSERIALNO1);
					elementb = findElementByID(ODFLinesData.getItemAt(j).PORTSERIALNO2);
					if(elementa && elementb){
						removeLinks(elementa.id.toString(),elementb.id.toString());
						var link1:Link = createLink(Node(elementa),Node(elementb), 0x005e9b,Consts.LINK_TYPE_PARALLEL);
						link1.setClient("linktype","matrix");
						link1.setClient("code",elementa.id.toString().split("-")[1]+":"+elementb.id.toString().split("-")[1]);
						link1.layerID=layer5.id;
						link1.setStyle(Styles.SELECT_STYLE,Consts.SELECT_STYLE_BORDER);
						link1.setStyle(Styles.SELECT_COLOR,"0x00FF00");
						link1.setStyle(Styles.SELECT_WIDTH,'5');
					}
				}
			}
		}
		...
		public function createFiberLink():void{
			for(var i:int =0;i<FiberLinesData.length;i++){
				...
				elementa = findElementByID(FiberLinesData.getItemAt(i).fibercode);
				if((FiberLinesData.getItemAt(i).aendodfport != null && FiberLinesData.getItemAt(i).aendodfport != "") && 
					...
					elementb = findElementByID(FiberLinesData.getItemAt(i).aendodfport);
					...
					elementb = findElementByID(FiberLinesData.getItemAt(i).zendodfport);
					...
					elementb = findElementByID(FiberLinesData.getItemAt(i).aendodfport);
					elementc = findElementByID(FiberLinesData.getItemAt(i).zendodfport);
				}
				...
				if(elementa != null && elementc != null){
					...
					link.setClient("linktype","fiber");
					link.setClient("code",elementa.id.toString().split("-")[1]+":"+elementc.id.toString().split("-")[1]);
					link.layerID=layer5.id; 
					link.setStyle(Styles.SELECT_STYLE,Consts.SELECT_STYLE_BORDER);
					link.setStyle(Styles.SELECT_COLOR,"0x00FF00");
					link.setStyle(Styles.SELECT_WIDTH,'5');
		...
		private function findElementByID(id:String):Follower{
			for(var i:int = 0; i < network.elementBox.datas.count; i++){
				...
				if(element is Follower){
					var foID:String = StringUtil.trim(String(Follower(element).id).split("-")[1]);
		...
		//根据两个端口编码删除连接关系
		private function removeLinks(port1:String,port2:String):void{
			for(var i:int = 0; i < network.elementBox.datas.count; i++){
				var element:IElement = network.elementBox.datas.getItemAt(i);
				if(element is Link && element.layerID == layer5.id){
					var portA:String = String(Link(element).getClient("code")).split(":")[0];
					var portZ:String = String(Link(element).getClient("code")).split(":")[1];
					if((portA == port1.split("-")[1] && portZ == port2.split("-")[1])
					  ||(portA == port2.split("-")[1] && portZ == port1.split("-")[1]))
		...
		public function CreateEquipmentModule(EquipPanelWidth:int,EquipPanelHeight:int,equipmentModel:EquipmentModule,from:String):void{
			...
			equipmentName = vendor+"$"+ model.replace("-","$");
			...
			if(equippackportdata){
				...
				for(var t:int = 0; t < equippackportdata.length; t++){
					...
					vmodel.status = equippackportdata[t].status;
				...
				equipment = createFollower(equipmentModel.EQUIPMENT_DATA.equipname,equipmentName,"", new Point(PositionX,PositionY),Consts.POSITION_TOP_TOP,"equipment-"+eid);
			...
			for(var k:int=0;k<groupsize;k++)
			{
				...
				if(from == "Initialization"){
					...
					for(var i:int= 0; i < acPorts.length;i++){
						if(acPorts.getItemAt(i).portserial == portserial){
							portposition = i+1;
							break;
						}
					}
					
					if(portposition >= parseInt(tempArr[0]) && portposition <= parseInt(tempArr[1])){
		 ...
		 * 四个个参数，id代表设备的名称，slotserial代表机槽序号 packserial机盘序号 type处理ODF端口的情况
		 * 返回一个ArrayCollection
		 * xiezhikui
		 */
		private function searchPorts(id:String,frameserial:String = "", slotserial:String = "",packserial:String = "",type:String=""):ArrayCollection{
						...
						if(tempArr.length == 2){
							if(type == "fiber"){
								if(Follower(follower.parent).followers){
									var parent:Follower = follower.parent as Follower;
									parent.setClient("state",follower.name);
									removeAllFollower(Follower(follower.parent).followers,"PORT_NODE");
									var rotationType:String = Follower(follower.parent).image.substring(8);
									addPortToFiber(parseInt(tempArr[0])-1,parseInt(tempArr[1]),follower,"fiber",follower.parent.getClient("code"),"Single",rotationType);
								}
							}else{
		...
		public function CreateODFModule(PositionX:int,PositionY:int,ModuleWidth:int,odfodmmodel:ODMMODULE,from:String):void{
			var odfodmsubModule:ArrayCollection = odfodmmodel.ODFODM_SUB_DATA;
			var dmmodel:DMModel = odfodmmodel.ODFODM_DATA;
			var ModuleName:String = dmmodel.dmname_std;
			var moduleCode:String = dmmodel.code;
			selectedTreeItem(moduleCode,"odf",true);
			
			var tempAC:ArrayCollection = new ArrayCollection();
			for(var i:int = 0; i < odfodmsubModule.length;i++){
				var submodule:ODFSUBMODULE  = odfodmsubModule.getItemAt(i) as ODFSUBMODULE;
				if(submodule.ODFODMPORT_DATA != null && submodule.ODFODMPORT_DATA.length > 0){
					var odfportModuleData:ArrayCollection = submodule.ODFODMPORT_DATA; 
					for(var j:int = 0; j < odfportModuleData.length; j++){
						var model:VolumeModel = new VolumeModel();
						model.portcode = odfportModuleData[j].odfportcode;
						model.porttype = "ZY23010499";
						model.portserial = odfportModuleData[j].portserial;
						model.status = odfportModuleData[j].status;
						model.name = odfportModuleData[j].port_name_std;
						model.linecount = odfportModuleData[j].linecount;
			...
			var MainPanel:Node ;
			if(dmmodel.castdirection != null && dmmodel.castdirection == "1"){  //竖向排列
				MainPanel= createFollower(ModuleName, "ODFODMV"+rowcount.toString(),"",new Point(PositionX, PositionY),Consts.POSITION_BOTTOM_BOTTOM,"odfodm-"+moduleCode);
//				MainPanel.width = (rowcount + 1) * 19;
				MainPanel.height = (colcount + 2) * 20;
			}else{
...
//				MainPanel.height = (rowcount + 1) * 19;
			...
			if(dmmodel.castdirection != null && dmmodel.castdirection == "1"){  //竖向排列
				for(var i:int = 1; i <= colcount; i++){
					...
					port.setLocation(PositionX,PositionY+20*(i-1));
				...
				for(var i:int = 0; i < odfodmsubModule.length;i++){
					var submodule:ODFSUBMODULE  = odfodmsubModule.getItemAt(i) as ODFSUBMODULE;
					if(submodule.ODFODMPORT_DATA != null && submodule.ODFODMPORT_DATA.length > 0){
						var label:Follower = new Follower();
						label.name = submodule.odfodmsub.label;
						label.setSize(20,20);
						label.parent = MainPanel;
						label.host = MainPanel;
						label.layerID = layer3.id;
						label.setLocation(PositionX+19*i+15,PositionY-20);
						label.setStyle(Styles.LABEL_COLOR, ColorKeys.WHITE);
						label.setStyle(Styles.LABEL_SIZE,14);
						label.setStyle(Styles.LABEL_BOLD,true);
						label.icon = "";
						label.image = "";
						dataBox.add(label);
						for(var j:int = 0; j < submodule.ODFODMPORT_DATA.length; j++){
							portCount++;
							port = new Follower("odf"+"-"+submodule.ODFODMPORT_DATA.getItemAt(j).odfportcode);
							port.layerID=layer3.id;
							if(submodule.ODFODMPORT_DATA.getItemAt(j).status =="ZY13100203"){
								image = "BADODFPORT";
							}else if (submodule.ODFODMPORT_DATA.getItemAt(j).linecount == 0) {
								image = "ODFVPORT";
							} else if (submodule.ODFODMPORT_DATA.getItemAt(j).linecount == 1) {
								image = "odfvyellow";
							} else if (submodule.ODFODMPORT_DATA.getItemAt(j).linecount == 2) {
								image = "odfvgreen";
							} else {
								image = "odfvred";
							}
							port.setClient("status",submodule.ODFODMPORT_DATA.getItemAt(j).status);
							port.setClient("linecount",submodule.ODFODMPORT_DATA.getItemAt(j).linecount);
							port.icon = image;
							port.image = image;
							port.parent = MainPanel;
							port.host = MainPanel;
							port.setClient("porttype","ZY23010499");
							port.setClient("code",submodule.ODFODMPORT_DATA.getItemAt(j).odfportcode);
							port.setClient("name",submodule.ODFODMPORT_DATA.getItemAt(j).port_name_std);
							port.setClient("roomcode",dmmodel.roomcode);
							port.setClient("stationcode", dmmodel.stationcode);
							port.setClient("castdirection","1");
							port.setSize(14,18);
							port.layerID = layer3.id;
							port.setLocation(PositionX+19*i+16,label.y+20*(j+1)+20);
							if(submodule.ODFODMPORT_DATA.getItemAt(j).circuit != null && submodule.ODFODMPORT_DATA.getItemAt(j).circuit != ""){
								port.toolTip="业务名称："+submodule.ODFODMPORT_DATA.getItemAt(j).circuit;
							}
							num++;
							dataBox.add(port);
						}
				...
				for(var i:int = 1; i <= colcount; i++){
					...
					port.host = MainPanel;
					port.parent = MainPanel;
					port.setLocation(PositionX+8+20*i,PositionY-20);
					port.layerID = layer3.id;
					dataBox.add(port);
				}
				for(var i:int = 0; i < odfodmsubModule.length;i++){
					...
					if(submodule.ODFODMPORT_DATA != null && submodule.ODFODMPORT_DATA.length > 0){
						...
						label.host = MainPanel;
						label.layerID = layer3.id;
						label.setLocation(PositionX,PositionY+19*i-8);
						label.setStyle(Styles.LABEL_COLOR, ColorKeys.WHITE);
						label.setStyle(Styles.LABEL_SIZE,14);
						label.setStyle(Styles.LABEL_BOLD,true);
						label.icon = "";
						...
						for(var j:int = 0; j < submodule.ODFODMPORT_DATA.length; j++){
							...
							port.setClient("roomcode",dmmodel.roomcode);
							port.setClient("stationcode", dmmodel.stationcode);
							port.setClient("castdirection","0");
							port.setSize(18,14);
							port.layerID = layer3.id;
							port.setLocation(label.x+10+20*(j+1),PositionY+19*(i+1)-3);
							if(submodule.ODFODMPORT_DATA.getItemAt(j).circuit != null && submodule.ODFODMPORT_DATA.getItemAt(j).circuit != ""){
								port.toolTip="业务名称："+submodule.ODFODMPORT_DATA.getItemAt(j).circuit;
							}
							num++;
...
//							if (count < portSize) {
//								portCount++;
//								port = new Follower("odf"+"-"+odfportModuleData.getItemAt(count).odfportcode);
//								port.layerID=layer3.id;
//								if(odfportModuleData.getItemAt(count).status =="ZY13100203"){
//									image = "BADODFPORT";
//								}else if (odfportModuleData.getItemAt(count).linecount == 0) {
//									image = "odf_port";
...
//									image = "odf_red";
//								}
//								port.setClient("status",odfportModuleData.getItemAt(count).status);
...
//									port.toolTip="业务名称："+odfportModuleData.getItemAt(count).circuit;
//								}
//								num++;
//								dataBox.add(port);
//							}
...
//				dispatchEvent(new Event("graphicsCompleted"));
		...
		public function CreateDDFModule(PositionX:int,PositionY:int,ModuleWidth:int,ddfddmmodel:DdfDdmModule,from:String):void{
			...
			{ 
				...
				for(var t:int = 0; t < ddfportModuleData.length; t++){
					...
					model.status =ddfportModuleData[t].status;
					model.porttype = "ZY13010499";
					model.linecount = ddfportModuleData[t].linecount;
					tempAC.addItem(model);
				}
				...
				for(var i:int=0;i < rowcount;i++)
				{
					for(var j:int =0;j<colcount;j++){
						...
						if(ddfportModuleData.length > mark){
							...
							portCount++;
							var ports:Follower = createFollower("", "", "ZY13010499", new Point(xLocation, yLocation), Consts.POSITION_BOTTOMLEFT,"ddf"+"-"+pccode);
							if(ddfportModuleData.getItemAt(mark).status == "ZY13100203"){
								ports.image= "BADDDFPORT";
							}else if (ddfportModuleData.getItemAt(mark).linecount == 0)
							...
							ports.setClient("status",ddfportModuleData.getItemAt(mark).status);
							...
							if(ddfportModuleData.getItemAt(mark).realycircuit != null && ddfportModuleData.getItemAt(mark).realycircuit != ""){
								ports.toolTip="业务名称："+ddfportModuleData.getItemAt(mark).realycircuit;
								if(ddfportModuleData.getItemAt(mark).oppositeport != null && ddfportModuleData.getItemAt(mark).oppositeport != ""){
									ports.toolTip += "\n对端端口：" +ddfportModuleData.getItemAt(mark).oppositeport;
								}
							}else if(ddfportModuleData.getItemAt(mark).circuit != null && ddfportModuleData.getItemAt(mark).circuit != ""){
								ports.toolTip="业务名称："+ddfportModuleData.getItemAt(mark).circuit;
							}else{
								ports.toolTip="";	
							}
						}
	...
	//				dispatchEvent(new Event("graphicsCompleted"));
		 ...
		 *@version ver.1.0
		 * */
		public function CreateFiberModule(PositionX:int,PositionY:int,ModuleWidth:int,ogmdata:OcableGraphicModule,rotationType:String,from:String,isrevolve:Boolean=false):void{
			...
			for(var t:int = 0; t < fiberdata.length; t++){
				var model:VolumeModel = new VolumeModel();
				model.portcode = isrevolve?fiberdata[t].portcode: fiberdata[t].fiberCode;
				model.portserial =isrevolve?fiberdata[t].portserial: fiberdata[t].fiberSerial;
				model.porttype = "fiber";
				model.status =fiberdata[t].status;
			...
			for(var j:int=0;j<groupsize;j++)
			{
				if(rotationType == "Down"){
					...
					tempY = equipment.y -equipment.height/2+20;
				}else if(rotationType == "Left"){
					if(groupsize == 1){
						tempY = equipment.y ;
					}else if(groupsize == 2){
						tempY = equipment.y - 25 + 50 * j;
					}else{
						tempY = equipment.y - 50*groupsize/2+25 + 50 * j;
					}
					tempX = equipment.x + equipment.width+40;
					...
					tempX = equipment.x-50;
				}
				node = createFollower(j*12+1+"-"+(((j+1)*12)<= fiberdata.length?(j+1)*12:fiberdata.length),"PORT_NODE","fiber", new Point(tempX,tempY),"",ocableCode+"-"+
				...
				if(rotationType == "Dowm"){
					node.setStyle(Styles.LABEL_POSITION, Consts.POSITION_BOTTOM_BOTTOM);
					myLink.setStyle(Styles.LINK_TYPE, Consts.LINK_TYPE_ORTHOGONAL_VERTICAL);
				}else if(rotationType == "Up"){
					node.setStyle(Styles.LABEL_POSITION, Consts.POSITION_TOP_TOP);
					myLink.setStyle(Styles.LINK_TYPE, Consts.LINK_TYPE_ORTHOGONAL_VERTICAL);
				}else if(rotationType == "Left"){
					node.setStyle(Styles.LABEL_POSITION, Consts.POSITION_LEFT_LEFT);
					myLink.setStyle(Styles.LINK_TYPE, Consts.LINK_TYPE_ORTHOGONAL_HORIZONTAL);
				}else if(rotationType == "Right"){
					node.setStyle(Styles.LABEL_POSITION, Consts.POSITION_RIGHT_RIGHT);
					myLink.setStyle(Styles.LINK_TYPE,Consts.LINK_TYPE_ORTHOGONAL_HORIZONTAL);
				}
				...
				if(!isRun){
					if(from == "Initialization"){
						...
						if(parseInt(tempPortserial) >= parseInt(tempArr[0]) && parseInt(tempPortserial) <= parseInt(tempArr[1])){
							...
							equipment.setClient("state",node.name);
						}
					}else{
						if(isrevolve){
							if(state == node.name){
		...
		private function addPortToFiber(startIndex:int,endIndex:int,node:Node,mark:String,name:String,from:String,rotationType:String=""):void{
			...
			{   
				...
				if(mark == "fiber"){
					if(rotationType == "Down"){
						...
						if(distinct == 1){
							...
							tempY =  node.y + 30 * tempInt -15;
						}else{
							tempY =node.y - 30*(distinct/2)+15 + 30*tempInt;
						}
						tempX = node.x + 60;
					}else if(rotationType == "Right"){
						if(distinct == 1){
					...
					if(rotationType == "Dowm"){
						follower.setStyle(Styles.LABEL_POSITION, Consts.POSITION_TOPRIGHT);
					}else if(rotationType == "Up"){
						follower.setStyle(Styles.LABEL_POSITION, Consts.POSITION_RIGHT_RIGHT);
					}else if(rotationType == "Left"){
						follower.setStyle(Styles.LABEL_POSITION, Consts.POSITION_TOPLEFT);
					}else if(rotationType == "Right"){
						follower.setStyle(Styles.LABEL_POSITION, Consts.POSITION_TOPRIGHT);
					}
					if(acPorts.getItemAt(i).status == "ZY13100203"){
						follower.image = "BADPORTICON";
					}else if(acPorts.getItemAt(i).linecount == "0"){
					...
					follower = createFollower(port.toString(),"",porttype, new Point(tempX,node.y + 60),Consts.POSITION_TOPRIGHT,"equipment-"+acPorts.getItemAt(i).portcode);
					follower.setClient("stationcode",node.getClient("stationcode"));
					if(acPorts.getItemAt(i).porttype == "ZY03070403"){
						if(acPorts.getItemAt(i).status == "ZY13100203"){
							follower.image = "BADPORT_RECT";
						}else if(acPorts.getItemAt(i).linecount == "0"){
				...
				follower.setClient("status",acPorts.getItemAt(i).status);
				follower.host = node;
...
//				dispatchEvent(new Event("graphicsCompleted"));
		...
		private function setFiberLineCount(fiberdata:ArrayCollection):void{
			if(fiberdata){
				for(var i:int = 0; i < acAllPorts.length; i++){
					if(acAllPorts[i].id == fiberdata[0].ocableCode){
						var tempAC:ArrayCollection = acAllPorts[i].data as ArrayCollection;
						for each(var obj:Object in tempAC){
							for(var j:int = 0; j < fiberdata.length; j++){
								if(obj.portcode == fiberdata[j].fiberCode){
									obj.linecount = fiberdata[j].linecount;
		...
		private function setEquipPortLineCount(wcgd:wireConfigurationGraphicsDataModule):void{
			...
			equippackportdata = equipmentModuleData.getItemAt(0).EQUIPPACK_PORT_DATA;
			if(equippackportdata){
				for(var i:int = 0; i < acAllPorts.length; i++){
					if(acAllPorts[i].id == equipmentModuleData.getItemAt(0).EQUIPMENT_DATA.equipcode){
						var tempAC:ArrayCollection = acAllPorts[i].data as ArrayCollection;
						for each(var obj:Object in tempAC){
							for(var j:int = 0; j < equippackportdata.length; j++){
								if(obj.portcode == equippackportdata[j].logicport){
									obj.linecount = equippackportdata[j].linecount;
									var follower:Follower =elementBox.getElementByID("equipment-" + obj.portcode) as Follower;
									if(follower != null){
										if(follower.getClient("porttype") == "ZY03070403"){
		...
		private function getODFMuduleHandler(event:ResultEvent):void{
			...
			if(wcgd.ODFODM_MODULE_DATA.length > 0){
				...
				if(isExist("odfodm-"+wcgd.ODFODM_MODULE_DATA.getItemAt(0).ODFODM_DATA.code,"id")){
					...
					for(var i:int = 0; i < acAllPorts.length;i++){
						if(acAllPorts[i].id ==wcgd.ODFODM_MODULE_DATA.getItemAt(0).ODFODM_DATA.code ){
							acAllPorts.removeItemAt(i);
							break;
						}
					}
		...
		private function getDDFModuleHandler(event:ResultEvent):void{
			...
			if(wcgd.DDFDDM_MODULE_DATA.length > 0){
				...
				if(isExist("ddfddm-"+wcgd.DDFDDM_MODULE_DATA.getItemAt(0).DDFDDM_DATA.code,"id")){
					...
					for(var i:int = 0; i < acAllPorts.length;i++){
						if(acAllPorts[i].id ==wcgd.DDFDDM_MODULE_DATA.getItemAt(0).DDFDDM_DATA.code ){
							acAllPorts.removeItemAt(i);
							break;
						}
					}
		...
		public function generateWireTree(event:ResultEvent):void
		{
			...
			if(folderList.elements().length() <= 0){
				Alert.show("在该局站内未找到相关资源","提示"); 
			}
			selectedTreeItem(this.dmcode,this.porttype == null?"":this.porttype== "ZY23010499"?"odf":"ddf",true);
		}
		...
		private function changeState():void{
			if(this.text.visible){
				...
				this.rightpanel.includeInLayout = true;
				this.linkButton.label=">>";
			}else{
				this.text.visible=true;
				this.text.includeInLayout=true;
				this.rightpanel.visible=false;
				this.rightpanel.includeInLayout=false;
				this.linkButton.label="<<";
			}
		}
		...
		private function drawPortRelationHandler(event:ResultEvent):void{
			...
			if(porttype == "ZY13010499"){
				...
				for(var i:int = 0;i<wcgd.EQUIPMENT_MODULE_DATA.length;i++){
					var tempPort:String = searchLinkPort(portcode,porttype,"equipment");
					if(isExist("equipment-"+wcgd.EQUIPMENT_MODULE_DATA.getItemAt(i).EQUIPMENT_DATA.equipcode,"id")){
						var packport:ArrayCollection = wcgd.EQUIPMENT_MODULE_DATA.getItemAt(i).EQUIPPACK_PORT_DATA ;
						for(var j:int =0; j<packport.length;j++){
							if(tempPort == packport[j].logicport){
