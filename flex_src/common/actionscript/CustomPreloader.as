...
{
//	import common.actionscript.gif.player.GIFPlayer;
	...
	{   
		...
		{   
            logo = new Loader();   
            logo.load(new URLRequest("assets/images/Loading_bg.png"));   
            addChild(logo);
			addChild(loadingBar);

//			var request:URLRequest = new URLRequest("assets/swf/login.swf"); 
//			player = new GIFPlayer(); 
//			player.load(request);                          
//			addChild(player); 
			
            var style:TextFormat = new TextFormat(null,null,0x000000,null,null,null,null,null,"left");   
            txt = new TextField();   
		...
		private function resize(e:Event):void{   
			logo.x = (stage.stageWidth - 500 )/2;   
			logo.y = (stage.stageHeight - 300 )/2;   
//			player.x = (stage.stageWidth - 40)/2;   
//			player.y = (stage.stageHeight + 80)/2; 
...
//			logo.y = (stage.stageHeight-185) *.5;
			loadingBar.x = (stage.stageWidth - 500) / 2 + 40;
			loadingBar.y = (stage.stageHeight)/2 + 30;
			txt.x = (stage.stageWidth - 500) / 2 + 40;
...
//			graphics.endFill();
		...
		private function load_progress(e:ProgressEvent):void{  
			txt.text = "加载中"+int(e.bytesLoaded/e.bytesTotal*100)+"%";
			
			var ratio:Number = e.bytesLoaded/e.bytesTotal;

			loadingBar.graphics.clear();
			loadingBar.graphics.beginFill(0x37e029);
			loadingBar.graphics.drawRect(0,0, ratio*400, 10);
			loadingBar.graphics.endFill();
			
//			txt.text = "正在加载" + ((ratio*100)>>0) + "%";
