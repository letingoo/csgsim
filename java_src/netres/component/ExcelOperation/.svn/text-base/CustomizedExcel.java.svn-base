...
public class CustomizedExcel {
	...
	public CustomizedExcel(ServletConfig servletConfig) {
		// ServletContext context =servletConfig.getServletContext();
		// this.LIMIT_SIZE =
		// Integer.valueOf(context.getInitParameter("LIMIT_SIZE"));
		// <context-param>
		this.LIMIT_SIZE = Integer.valueOf(servletConfig
				.getInitParameter("LIMIT_SIZE"));
	}

	public void WriteExcel(String filename, String title, String[] labels,
			List content) throws IOException, WriteException {
		WorkbookSettings ws = new WorkbookSettings();
		ws.setLocale(new Locale("en", "EN"));
		WritableWorkbook workbook = Workbook.createWorkbook(new File(filename),
				ws);
		if (content.size() > LIMIT_SIZE) {
			int page = 0;
			if (content.size() % LIMIT_SIZE == 0) {
				page = content.size() / LIMIT_SIZE;
			} else {
				page = content.size() / LIMIT_SIZE + 1;
			}
			for (int i = 1; i <= page; i++) {
				WritableSheet wsheet = workbook.createSheet("sheet" + i, i - 1);
				WriteTitleSheet(wsheet, title, labels.length);
				WriteLabelsSheet(wsheet, filename, labels);
//				if(i == 1){
//					WriteContentSheet(workbook, wsheet, content.subList(0,98));
//				}else{
//					WriteContentSheet(workbook, wsheet, content.subList((i - 1)* LIMIT_SIZE - 2 * (i - 1),i * LIMIT_SIZE > content.size() ? content.size() : (i * LIMIT_SIZE - i * 2 )));
//				}
				WriteContentSheet(workbook, wsheet, 
						content.subList((i - 1)* LIMIT_SIZE,
								i * LIMIT_SIZE > content.size() ? content.size() : i* LIMIT_SIZE));
			}
		} else {
	...
	private void WriteTitleSheet(WritableSheet wsheet, String title, int in)
	...
	private void WriteLabelsSheet(WritableSheet wsheet, String filename,
			String[] labels) throws IOException, WriteException {

		WritableFont wfont = new WritableFont(WritableFont.TIMES, 9,
				WritableFont.BOLD, false,
				jxl.format.UnderlineStyle.NO_UNDERLINE, jxl.format.Colour.WHITE);
	...
	private void WriteContentSheet(WritableWorkbook workbook,
			WritableSheet wsheet, List content) throws IOException,
			WriteException {
...
//		System.out.println("----"+content.get(0).toString());
		for (int i = 0; i < content.size(); i++) {
			...
			strcon[strcon.length - 1] = strcon[strcon.length - 1].substring(0,
					strcon[strcon.length - 1].length() - 1);
	...
	public void UpdateExcel(String filename) throws IOException, WriteException {
		...
		try {
			...
			sheet.addCell(new Label(2, 4,
					"三峡左岸至龙泉换流站安控（1） （1＋1切换装置）通道（1） 接入方式", contentFormat));
			// 结束更新excel中的cell
			workbook.write();
			workbook.close();
		} catch (Exception e) {
	...
	public void UpdateExcelCellBySheet(String filename, int sheetnum)
			throws IOException, WriteException {
		WritableFont wfontLable = new WritableFont(WritableFont.TIMES, 12,
				WritableFont.NO_BOLD, false,
				jxl.format.UnderlineStyle.NO_UNDERLINE, jxl.format.Colour.BLACK);
		...
		try {
			...
			sheet.addCell(new Label(4, 3, "国网信通公司", contentFormat));
			sheet.addCell(new Label(2, 4,
					"三峡左岸至龙泉换流站安控（1）gg （1＋1切换装置）通道（1） 接入方式", contentFormat));
			// 结束更新excel中的cell
			workbook.write();
			workbook.close();
		} catch (Exception e) {
	...
	/**
	 * 只支持png类型的图片 para:excelfilename 需要更新的excel的路径 sheetnum 需要更新的第几个tab
	 * imagepath 写入excel的图片来源地址 imagerow imagecol 图片要写到excel的列 行 开始
	 * 
	 */
	...
	public void UpdateExcelImageByWidthAndHeight(String excelfilename,
			int sheetnum, String imagepath, int imagerow, int imagecol,
			int width, int height) throws IOException, WriteException {
		WritableFont wfontLable = new WritableFont(WritableFont.TIMES, 12,
				WritableFont.NO_BOLD, false,
				jxl.format.UnderlineStyle.NO_UNDERLINE, jxl.format.Colour.BLACK);
		WritableCellFormat contentFormat = new WritableCellFormat(wfontLable);
		...
		try {
			...
			WritableImage image = new WritableImage(imagerow, imagecol, width,
					height, fileImage);
			sheetpic.addImage(image);
			workbook.write();
			workbook.close();
	...
	public void WriteExcel_Alarm(String filename, String title, String[] labels,
			List content) throws IOException, WriteException {
		WorkbookSettings ws = new WorkbookSettings();
		ws.setLocale(new Locale("en", "EN"));
		WritableWorkbook workbook = Workbook.createWorkbook(new File(filename),
...
//					WriteContentSheet(workbook, wsheet, content.subList((i - 1)* LIMIT_SIZE - 2 * (i - 1),i * LIMIT_SIZE > content.size() ? content.size() : (i * LIMIT_SIZE - i * 2 )));
//				}
				WriteContentSheet_Alarm(workbook, wsheet, 
						content.subList((i - 1)* LIMIT_SIZE,
								i * LIMIT_SIZE > content.size() ? content.size() : i* LIMIT_SIZE));
			}
		} else {
			...
			WriteTitleSheet(wsheet, title, labels.length);
			WriteLabelsSheet(wsheet, filename, labels);
			WriteContentSheet_Alarm(workbook, wsheet, content);
		}
		workbook.write();
		workbook.close();
	}
	
	private void WriteContentSheet_Alarm(WritableWorkbook workbook,
			WritableSheet wsheet, List content) throws IOException,
			WriteException {
		...
		for (int i = 0; i < content.size(); i++) {
			// wsheet.setRowView(i+2, 600); //设置行高
			String[] strcon = content.get(i).toString().split("##");
//			strcon[0] = strcon[0].substring(0, strcon[0].length());
//			strcon[strcon.length - 1] = strcon[strcon.length - 1].substring(0,
//					strcon[strcon.length - 1].length() - 1);
			for (int k = 0; k < strcon.length; k++) {
				wsheet.setColumnView(k, 15);
