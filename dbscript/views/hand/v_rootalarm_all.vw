create or replace view v_rootalarm_all as
Select distinct
a.alarmnumber,
...
a.dealpart,                                                                                                         --处理部门
a.dealresult,                                                                                                       --处理方式
dealmethod.xtxx dealresultzh,                                                                                        --处理方式描述
e.run_unit,                                                                                                         --维护单位
a.iscleared,                                                                                                        --是否清除
a.clearperson,                                                                                                      --清除人
...
a.firstarrivetime,
c.username,
decode(ca.companyalarmcnt,null,0,ca.companyalarmcnt) companyalarmcnt
From alarmInfo a,Station s,equipment e,
alarmdefine ad,
en_adapter_domain ead,
xtbm xvendor,
xtbm spe,
v_alarmrelatecircuit c,
xtbm dealmethod,
v_mnt_companyalarm_total ca
where a.probablecause=ad.id
  ...
  and a.alarmnumber = c.alarmnumber(+)
  and a.belongsubshelf='1'
  and a.dealresult = dealmethod.xtbm(+)
  and a.alarmnumber=ca.alarmnumber(+)
  --and a.iscleared=0
